...
import wizrole.hoservice.view.GestureLockView.Mode;
 ...
 *
 *
 * 验证手势密码的合法性---【独立于 修改和新建手势密码】
 */
public class GestureLockViewGroup_OnResume extends RelativeLayout {
...
//	public boolean tOrFalse;

	public GestureLockViewGroup_OnResume(Context context, AttributeSet attrs){
	...
	public boolean onTouchEvent(MotionEvent event){
		...
		case MotionEvent.ACTION_UP:
			...
			if (mOnGestureLockViewListener != null && mChoose.size() > 0){
				...
				mOnGestureLockViewListener.sendData(mChoose);
				//验证密码是否正确  // 此处为验证密码合法性使用
				if(checkAnswer()){
					TrueItemMode();	//绿色
					reset();//还原界面
				}else{
...
//		mPath.reset();
	...
	public interface OnGestureLockViewListener{
		 ...
		 * @param mChoose
         */
		public void sendData(List<Integer> mChoose);
